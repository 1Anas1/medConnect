...
exports.handleWebRTCSignal = async (req, res) => {
  try {
    const { appointmentId, from, to, type, sdp, candidate } = req.body;

    if (!appointmentId || !from || !to || !type) {
    ...
    res.status(200).json({ message: 'Signal stored successfully', signalId });
  } catch (error) {
    logger.error('Error in handleWebRTCSignal:', error);
    res.status(500).json({ message: 'Server error while storing signal' });
  }
};

/**
 * GET /api/webrtc/signal?appointmentId=xxx&forUserId=yyy
 * Polling version - not ideal but fallback if WebSocket not available
 */
exports.pollSignals = async (req, res) => {
  try {
    const { appointmentId, forUserId } = req.query;

    if (!appointmentId || !forUserId) {
      return res.status(400).json({ message: 'Missing required parameters' });
    }

    const params = {
      TableName: TABLES.SIGNALS,
      IndexName: 'appointmentId-to-index',
      KeyConditionExpression: 'appointmentId = :aId and #to = :toUser',
      ExpressionAttributeValues: {
        ':aId': appointmentId,
        ':toUser': forUserId
      },
      ...
      ScanIndexForward: true // newest last
    };

    const result = await dynamoDB.query(params).promise();

    res.status(200).json({ messages: result.Items });
  } catch (error) {
    logger.error('Error in pollSignals:', error);
    res.status(500).json({ message: 'Failed to retrieve signaling messages' });
  }
};